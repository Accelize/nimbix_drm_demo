...
#include <fpga_pci.h>
#include <fpga_mgmt.h>
#define FPGA_PROG_CMD std::string("sudo fpga-load-local-image -I agfi-0c4ab7b3438ea5e5b -S ")
...
{
    gDrmMutex.lock();
    if(fpga_pci_peek(gHandle[slotID], DRM_BASE_ADDRESS+addr, value)) {
...
{
    gDrmMutex.lock();
    if(fpga_pci_poke(gHandle[slotID], DRM_BASE_ADDRESS+addr, value)) {
...
{   
	gDrmMutex.lock();
    if(fpga_pci_peek(gHandle[slotID], USER_IP_BASE_ADDRESS+addr, value)) {
        std::cout << __FUNCTION__ << ": Unable to read from the fpga ! " << std::endl;
        gDrmMutex.unlock();
        return 1;
    }
